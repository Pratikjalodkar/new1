...
from .permissions import IsAdminUser   # Custom permission for admin access
...
def get_orders_by_customer(request, customerId):
    try:
        ...
        for order in paginated_orders:
            products = []
            for order_item in order.order_items.all():  # Use the related name defined in OrderItem
                products.append({
                    "product_id": order_item.product.id,
                    "name": order_item.product.name,
                    "quantity": order_item.quantity,
                    "price": str(order_item.price),  # Convert Decimal to string for JSON serialization
                })
        ...
        return paginator.get_paginated_response(order_data)  # Return paginated response
    except User.DoesNotExist:
        return Response({"message": "User  not found."}, status=status.HTTP_404_NOT_FOUND)